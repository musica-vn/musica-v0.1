...
- Map modules nghiệp vụ (IAM, Users, Products, Catalog, Checkout, Certificates, Dashboard) sang thứ tự triển khai tối thiểu để chạy end-to-end.
...
1) IAM & RBAC → 2) Users → 3) Products (ingestion) → 4) Catalog → 5) Checkout → 6) Certificates → 7) Dashboard.
